...
from util.dir import (CONFIG_FILENAME, CV_MODELS_DIR, DATA_HIST_DIR, MODELS_DIR, NEG_DATA_DIR, POS_DATA_DIR, makedir)
...
def train(model_dir: str, cv_iters: int) -> None:
    ...
    for cv_i in range(1, cv_iters + 1):
        ...
        x_trn, y_trn = load_data(
            ...
            os.path.join(POS_DATA_DIR, "bass_motif", "aa_pre_pad", f"bass_ctm_motif_trn{cv_i}_aa_pre_pad.fa")
        )
        x_val, y_val = load_data(
            os.path.join(NEG_DATA_DIR, "PB40", f"PB40_1z20_clu50_val{cv_i}.fa"),
            os.path.join(POS_DATA_DIR, "bass_motif", "aa_pre_pad", f"bass_ctm_motif_val{cv_i}_aa_pre_pad.fa")
        )

        # Pad protein sequences
        T = len(max(x_trn, key=len))
        x_trn = pre_pad_protein_sequences(x_trn, T)
        x_val = pre_pad_protein_sequences(x_val, T)
