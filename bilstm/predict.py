...
from util.dir import (CONFIG_FILENAME, CV_MODELS_DIR, DATA_PRED_DIR, MODELS_DIR, NEG_DATA_DIR, POS_DATA_DIR, SEP, makedir)
from util.json import from_json, to_json
from util.loader import read_fasta
from util.tokenizer import load_tokenizer


TST_SETS_FILEPATHS = [
    os.path.join(NEG_DATA_DIR, "CsgA", "csga2203_nr40.fa"),
    os.path.join(NEG_DATA_DIR, "CsgA", "csga2203ER4_nr40.fa"),
    ...
    os.path.join(POS_DATA_DIR, "bass_domain", "bass_c01_ntm_domain_test.fa"),
    os.path.join(POS_DATA_DIR, "bass_domain", "bass_c02_ntm_domain_test.fa"),
    os.path.join(POS_DATA_DIR, "bass_domain", "bass_c03_ntm_domain_test.fa"),
    os.path.join(POS_DATA_DIR, "bass_domain", "bass_c06_ntm_domain_test.fa"),
...
def predict(model_dir: str) -> None:
    ...
    for set_filepath in TST_SETS_FILEPATHS:
        ...
        # Pad sequences
        x_tst = tf.keras.preprocessing.sequence.pad_sequences(x_tst, maxlen=T, padding="post")

        y_pred = []
        for i, model_filepath in enumerate(cv_models_filepaths):
            # Load model
            model: tf.keras.models.Model = tf.keras.models.load_model(model_filepath)

            # Predict
            y_pred.append(model.predict(x_tst, batch_size=16).flatten()) # [[1], [1], ..., [1]] -> [1, 1, ..., 1]
